...
function validateReceipt(receipt, adapters, destinations, pathApi = path) {
  ...
  for (const adapter of adapters) {
    ...
    const expectedDestination = destinations[adapter.id];
    ...
    if (!samePath(record.destination, expectedDestination, pathApi)) {
    ...
    for (const skill of record.skills) {
      ...
      seen.add(skill.name);
      const expectedPath = pathApi.join(expectedDestination, skill.name);
