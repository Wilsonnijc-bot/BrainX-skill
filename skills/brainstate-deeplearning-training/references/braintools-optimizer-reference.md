...
Use this when a BrainState training task needs optimizer or learning-rate-scheduler selection beyond the default `braintools.optim.Adam` pattern.
...
- Keep gradient computation in `brainstate.transform.grad(...)` and apply updates through the optimizer update path already used by the training skill.
- Use this reference for selecting optimizer families, scheduler families, or external optimization routes; use the main training skill for loss/grad/JIT loop structure.
