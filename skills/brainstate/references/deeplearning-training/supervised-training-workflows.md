# Supervised Training Workflows Parent Reference

Use this nested reference for complete BrainState training workflows: losses, metrics, evaluation, gradient clipping, randomness, optimizer registration and updates, transformed training steps, and full epoch loops. Start from `skills/brainstate/SKILL.md`, then open only the deeper references routed here when the task needs them.

## Parent-reference boundary

This semantic parent reference replaces a generic directory landing file. `skills/brainstate/SKILL.md` routes here for complete supervised-training work; this parent then selects only the transform, parameter, randomness, optimizer, layer, and script references needed by the task. Those conditional children must not be loaded merely because training was mentioned.
...
• tiny RNG block
Use `brainstate.random.seed(seed)` once for reproducible training examples/tests. Use `brainstate.random` for random batches, synthetic data, stochastic layers, dropout/noise, and random spike data. Route advanced stochastic/dropout/fit-mode/randomness issues to `references/brainstate-randomness-reproducibility/randomness-and-reproducibility.md`.
...
## Conditional child-reference routes
...
references/deeplearning-training/references/scripts/integrator_rnn.py
...
[Training Spiking Neural Networks](https://brainx.chaobrain.com/brainstate/examples/brain_dynamics/snn_training.html)

**Source mirrored:** https://brainx.chaobrain.com/brainstate/examples/brain_dynamics/snn_training.html
**Purpose:** official longer example-gallery workflow for SNN training variants; no local duplicate is bundled.
