...
If independent state is required, use BrainState `vmap` state-axis controls such as `state_in_axes`/`state_out_axes` according to the local API. Read `transformation-vmap-expansion.md` before editing exact arguments.
...
Use `grad(loss_over_batch)` for normal batch training. Use `vmap(grad(single_loss))` only for per-sample gradients, influence functions, uncertainty, or diagnostics; it can be memory-heavy. Read `transformation-grad-expansion.md` before exact state/argument gradient edits.
...
- `skills/brainstate-transformations-core/SKILL.md`: before any nontrivial BrainState transform rewrite.
